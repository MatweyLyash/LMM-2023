...
namespace Gen
{
	...
	void Generator::Head()  // подключение lib и прототипы функций 
	{
		...
		out << "includelib D:\\Desktop\\Ёкзы\\LMM-2023\\Debug\\StaticLibrary.lib\n";
	...
	void Generator::Code() // код
	{
		...
		for (int i = 0; i < lexT.size; i++)
		{
			...
			case LEX_FUNCTION:
				...
				break;
			case LEX_ID:
				if (idT.table[lexT.table[i].idxTI].idType == IT::F) // если идентификатор функци€ 
				{
					int count = 0;
